...
- **关闭电池优化引导**: 在桌面设置里提供"关闭电池优化"引导, 改善 MIUI / HyperOS 上后台安装/卸载应用后桌面图标不刷新的问题
...
- **MIUI / HyperOS 后台策略限制**: 在系统设置里卸载/安装应用后桌面图标可能不及时刷新. 处理方式: 在桌面设置中开启"关闭电池优化"引导并完成设置
...
构建管线: 编辑 smali / 资源 → `apktool b` → 签名 → `adb install`.
